...
const API_BASE_URL = COMPLIANCE_API_BASE;
...
const ThreatComplianceDetail = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    const config =
      statusConfig[status || ''] || { label: status || '미확인', className: 'bg-gray-100 text-gray-600' };
    return (
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          <h2 className="text-lg font-bold text-gray-900">AWS 보안 통제 매핑 ({totalItems}개)</h2>
        ...
        {totalItems === 0 ? (
          ...
          <div className="space-y-4">
            {paginatedMappings.map((mapping, idx) => {
              ...
              const globalIndex = pageSize === 'all' ? idx : (page - 1) * effectivePageSize + idx;
