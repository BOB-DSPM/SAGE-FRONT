// DataTarget.js - "AWS 리소스 인벤토리" 박스만 테두리 추가 (기능/레이아웃 변경 없음)
...
import { AEGIS_API_BASE, COLLECTOR_API_BASE } from '../config/api';

const ANALYZER_API_BASE = AEGIS_API_BASE;
const COLLECTOR_API = COLLECTOR_API_BASE;
...
const DataTarget = ({ activeTab }) => {
  ...
  const handleSendToAnalyzer = async () => {
    ...
    try {
      const selectedItems = inventoryData.filter(item => selectedResources.has(item.id));
      ...
      try {
        ...
        console.error('✗ Step 2 실패: 위협 식별 결과 조회', error);
      }
      ...
      try {
        ...
        console.error('✗ Step 3 실패: RDS-S3 교차 검증', error);
      ...
      try {
        ...
        console.error('✗ Step 4 실패: 교차 검증 리포트 조회', error);
      }
