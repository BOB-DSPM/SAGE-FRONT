...
## 시스템 내 역할
- DSPM 전체 플랫폼의 **시각화 게이트웨이**로, 수집·분석된 데이터 보안 상태를 한눈에 보여주고 정책·알림을 제어합니다.
- `Data Target`, `Aegis`, `Lineage`, `Threat Compliance`, `Opensource` 등 백엔드 서비스들의 결과를 집약해 **운영자가 조치할 수 있는 단일 창구**를 제공합니다.
- 인증/세션 정보(`sessionService.js`)를 기준으로 사용자별 접근 권한을 분리해 **다른 서비스로 라우팅하기 전의 프런트 보안 경계** 역할을 합니다.

## 필요한 구성 이유
- **런타임 환경 변수 기반 설정**: 고객별 API 엔드포인트가 상이하므로, `public/env-config.js`와 `docker-entrypoint.sh`로 `REACT_APP_*` 값을 컨테이너 기동 시 주입해 빌드 재생성 없이 배포 환경을 바꿀 수 있습니다.
- **비루트 NGINX 실행**: AWS Marketplace 요구 사항 및 보안 모범 사례에 따라 컨테이너 내 웹 서버를 비루트로 실행해 호스트 영향 범위를 최소화합니다.
- **정적 자격 증명 배제**: API 키 등을 코드에 포함하지 않고 외부 비밀 관리자를 통해 전달하도록 해 공급망 노출 위험을 낮춥니다.
- **모듈형 서비스 호출**: `services/` 하위 API 클라이언트를 분리해 백엔드 서비스 증감·교체 시 프런트 수정 범위를 최소화하고, 장애 시 특정 모듈만 격리할 수 있습니다.
- **재사용 가능한 UI 컴포넌트**: `components/`를 공통화해 기능 확장 시 UI/UX를 일관되게 유지하며, 성능 최적화(코드 스플리팅·지연 로드)에 유리합니다.
